...
## 2026-04-22 - Float Validation Bypass via NaN/Inf
**Vulnerability:** Input validation functions used `qty <= 0` and similar bounds checks without ensuring the float is finite. This allowed attackers to bypass bounds checks by providing `float('nan')` or `float('inf')` which evaluate to False in `>= 0` or similar inequality tests.
**Learning:** Inequality bounds checking on floats is insufficient. `NaN` breaks normal boolean algebra for comparisons. Always ensure inputs are finite numbers.
**Prevention:** Always check `math.isfinite(val)` on parsed floating-point inputs before applying normal boundary checks like `val <= 0`.
