...
**Vulnerability:** Input validation logic for numbers in `src/validation.py` could be bypassed using `float('nan')` or `float('inf')`. Because `nan <= 0` is False, the value could bypass minimum value checks and potentially crash the bot or cause unintended downstream effects.
**Learning:** Using `float()` to parse user inputs does not automatically reject `NaN` or `Inf`. Special floating point values must be explicitly checked.
**Prevention:** Always use `math.isnan()` and `math.isinf()` (or `math.isfinite()`) to validate that parsed floating point numbers are actually finite numerical values before applying further bounds checking.
