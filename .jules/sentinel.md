...
## 2024-05-24 - [NaN/Inf Validation Bypass]
**Vulnerability:** User inputs that resolve to `float('nan')` or `float('inf')` bypassed minimum and maximum numerical boundary validation checks in Python.
**Learning:** In Python, any comparison operator (`<`, `>`, `<=`, `>=`) against `NaN` evaluates to `False`. Thus, boundary validations written like `if value < MIN or value > MAX` will silently evaluate to `False` allowing the `NaN` value to slip through the check.
**Prevention:** Explicitly sanitize floats with `math.isnan()` and `math.isinf()` (or handle them via string/decimal checks) prior to applying standard comparative boundary validations.
