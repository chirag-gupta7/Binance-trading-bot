...
class OrderValidator:
    ...
    def validate_quantity(quantity: float, symbol: str = None) -> float:
        ...
        if math.isnan(qty) or math.isinf(qty):
            raise ValidationError(f"Quantity must be a valid number, got: {qty}")
    ...
    def validate_price(price: float, symbol: str = None) -> float:
        ...
        if math.isnan(p) or math.isinf(p):
            raise ValidationError(f"Price must be a valid number, got: {p}")
    ...
    def validate_stop_price(stop_price: float, entry_price: float, side: str) -> float:
        ...
        if math.isnan(stop) or math.isinf(stop) or math.isnan(entry) or math.isinf(entry):
            raise ValidationError("Stop price and entry price must be valid numbers")
    ...
    def validate_percentage(percentage: float) -> float:
        ...
        if math.isnan(pct) or math.isinf(pct):
            raise ValidationError(f"Percentage must be a valid number, got: {pct}")
