    ...
    def validate_quantity(quantity: float, symbol: str = None) -> float:
        ...
        try:
            ...
            if not math.isfinite(qty):
                raise ValidationError(f"Quantity must be finite, got: {quantity}")
    ...
    def validate_price(price: float, symbol: str = None) -> float:
        ...
        try:
            ...
            if not math.isfinite(p):
                raise ValidationError(f"Price must be finite, got: {price}")
        except (TypeError, ValueError):
    ...
    def validate_stop_price(stop_price: float, entry_price: float, side: str) -> float:
        ...
        try:
            ...
            if not math.isfinite(stop) or not math.isfinite(entry):
                raise ValidationError("Stop price and entry price must be finite")
        except (TypeError, ValueError):
    ...
    def validate_percentage(percentage: float) -> float:
        ...
        try:
            ...
            if not math.isfinite(pct):
                raise ValidationError(f"Percentage must be finite, got: {percentage}")
        except (TypeError, ValueError):
